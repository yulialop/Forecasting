...
The `auto.arima()` function in R automatically selects the optimal ARIMA parameters for each time series data (IDs 1001 to 1100) from the M3 competition data set based on the corrected Akaike Information Criterion (AICc). However, it may not effectively capture complex seasonal patterns or structural changes in the data, leading to suboptimal forecasts in certain cases. The automatic ARIMA model may also struggle with noisy or irregular data, requiring additional preprocessing or model refinement to improve forecasting accuracy. Appendix \@ref(appendix-35) contains a comprehensive list of automatically fitted ARIMA models for each series along with their respective forecasted values
...
## Code dispaying series ID 1394 of the M3 competition data set  {#appendix-1}
...
## Code calculating measures, such as MAE, RMSE, and MAPE, to evaluate the accuracy of the the exponential smoothing (ETS (M, Ad, M)) model to forecast future data {#appendix-21}
...
## Code printining summaries of automatically fitted ARIMA models for each series (IDs 1001 to 1100) using `auto.arima()` function and printing respective forecasted values. {#appendix-35}
#Unhide comments!
...
## Code computing the average Mean Absolute Percentage Error (MAPE) and Symmetric Mean Absolute Percentage Error (sMAPE) for the ARIMA model, and comparing its performance with the Theta and Damped Exponential Smoothing models, which serve as benchmarks (bemchmark models fitted in Appendix \@ref(appendix-35)) {#appendix-36}
