...
```{r echo=FALSE}
cat("Table 2. Summary of the training data")

summary(training_data)
```

The series exhibits a downward trend before 1966, followed by a clear upward trend starting in 1966, indicating an increase in the unemployed since then. A strong seasonal pattern is evident, with peaks and troughs corresponding to particular times of the year (Q1 and Q3, respectively), suggesting likely annual seasonality influenced by various factors related to economic activity, societal habits, and institutional schedules.
...
The lag plot displays scatter plots of quarterly Canadian unemployment, revealing a strongly positive relationship at lag 4, reflecting the strong seasonality in the data:

```{r echo=FALSE}
gglagplot(training_data) +
  ggtitle("Lagged scatterplots: quarterly unemployment") +
    theme(legend.position = "bottom",
          axis.text.x = element_text(angle = 45, hjust = 1))
```

As the data are both trended and seasonal, we observe the slow decay in autocorrelation associated with the trend component alongside more significant spikes at lags matching the seasonal period:

```{r echo=FALSE, fig.width=6, fig.height=3.5}
ggAcf(training_data, main = "ACF: unemployment in Canada in 1962-1973")
```

As the data exhibits both trend and seasonality, decomposition can be a valuable insight by separating the time series into trend, seasonality, and residual components. Upon decomposing the time series, the multiplicative type of decomposition effectively subtracts the seasonal and trend-cycle components from the training data:
...
Detecting outliers in time series data is critical as they can significantly impact analysis and forecasts. 
...
The residuals appear to be normally distributed, and there are no apparent patterns in the residual plots:
...
However, the residuals' Autocorrelation Function plot reveals significant autocorrelation at lag 1, suggesting a violation of the assumption of independence.

Subsequently, the Ljung-Box test confirms significant autocorrelation in the residuals at lag 1, indicating that a time series model explicitly addressing autocorrelation, such as an ARIMA model, may offer a better fit.
...
The Ljung-Box test suggests that there may be some remaining patterns or information in the residuals that the model has not captured, as the p-value (0.07898) is greater than the conventional significance level of 0.05.
...
We can use various evaluation metrics, such as MAE, RMSE, and MAPE, to evaluate the accuracy of the regression forecast for future data. 
...
When evaluating the forecasting accuracy of the exponential smoothing model with the Holt-Winters method against future data, we observe that while the future data aligns with the 95% confidence interval for the first four quarters, it surpasses the forecasted range thereafter.
...
This discrepancy makes it challenging to directly assess the model’s forecasting performance solely based on this plot. Therefore, we conduct residual analysis to obtain a more objective evaluation and examine various evaluation metrics such as MAE, RMSE, and MAPE.
...
Subsequently, the first and seasonal differencings are applied to the historical data, followed by an examination of the ACF/PACF plots.
...
The efficacy of differencing is further confirmed by plotting the differenced historical data together with performing the ADF test, indicating stationarity of the differenced series, while the KPSS test fails to reject the null hypothesis of stationarity.
...
Residual analysis reveals normally distributed residuals with no significant autocorrelation, further affirming the adequacy of the ARIMA model.
...
Upon forecasting performance evaluation against future data, the ARIMA model exhibits reasonably low errors and bias, indicating satisfactory performance, albeit challenges in predicting extraordinary spikes in future data.
...
The below metrics suggest that the model’s forecasts have relatively low errors and bias, indicating reasonably good performance, considering the future data had extraordinary spikes.
...
evaluation_metrics <- data.frame(
  Model = c("ARIMA", "Expotential Smoothing", "Regression"),
...
The primary objective of this section of the report is to select, automatically fit, and evaluate three distinct statistical models for forecasting quarterly time series of IDs from 1001 to 1100 from the M3 competition data set. 
...
**Appendix contains a comprehensive list of automatically fitted ARIMA models for each series along with their respective forecasted values** [uncomment printing summary and forecasts]
...
# Store evaluation metrics for each model in a data frame
theta_batch_evaluation_metrics <- data.frame(
...
# Print the evaluation metrics for comparison
print(theta_batch_evaluation_metrics, row.names = FALSE)

# Select the model with the lowest values for MAPE
theta_batch_best_model_mape <- theta_batch_evaluation_metrics[which.min(theta_batch_evaluation_metrics$MAPE), ]

# Select the model with the lowest values for sMAPE
theta_batch_best_model_smape <- theta_batch_evaluation_metrics[which.min(theta_batch_evaluation_metrics$sMAPE), ]

# Print the best model
cat("Best model based on MAPE:", theta_batch_best_model_mape$Model, "\n")
cat("Best model based on sMAPE:", theta_batch_best_model_smape$Model, "\n")
...
## Automatic Error-Trend-Seasonality Model (ETS)

The `ets()` function in R automatically selects the optimal ETS model (Error, Trend, Seasonality) based on each series' data characteristics, such as the presence of trend and seasonality. The fitted ETS model contains information about the estimated parameters, including the level, trend, and seasonal components for each series and any additional settings or options specified during the fitting process. However, it may be computationally intensive for large-scale forecasting tasks and could produce suboptimal results if the underlying data contains irregular patterns or outliers.

**Appendix contains a comprehensive list of automatically fitted ETS models for each series along with their respective forecasted values**[uncomment printing summary and forecasts]
...
Similarly to the auto ARIMA model, we calculate the average Mean Absolute Percentage Error (MAPE) and Symmetric Mean Absolute Percentage Error (sMAPE) for the ETS forecasting method and compare them with the metrics for Theta and Damped Exponential Smoothing models as benchmarks.
...
## Exponential smoothing state space model with Box-Cox transformation, ARMA errors, Trend and Seasonal components Model (TBATS)

The TBATS model is renowned for its capability to handle multiple seasonalities, trends, and complex patterns in time series data. It is particularly useful for data sets with intricate seasonal patterns and irregular trends. It is preferred for automatic forecasting tasks where data may exhibit multiple seasonalities and nonlinear patterns. However, it may suffer from computational intensity, especially with large data sets, and might require tuning parameters for optimal performance in certain cases.
...
**Appendix contains a comprehensive list of automatically fitted TBATS models for each series along with their respective forecasted values** [uncomment printing summary and forecasts]
...
Similar to the auto ARIMA and ETS models, we compute the average Mean Absolute Percentage Error (MAPE) and Symmetric Mean Absolute Percentage Error (sMAPE) for the TBATS forecasting method and compare them with the metrics for Theta and Damped Exponential Smoothing models as benchmarks.
...
The competitive edge of the Damped Exponential Smoothing model over automatic ARIMA, ETS, and TBATS models for batch forecasting can be attributed to the factor highlighted by Koning et al. (2005) that the complexity of forecasting methods does not always correlate with forecast accuracy.
